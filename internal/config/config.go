...
func Parse(args []string, stderr io.Writer) (*Config, error) {
	...
	fs.IntVar(&cfg.Retries, "retries", 2, "Number of retries per record type")
...
func PrintHelp(stdout io.Writer) {
	...
	fmt.Fprintln(stdout, "  -output string         Output file path")
	...
	fmt.Fprintln(stdout, "  -retries int           Number of retries per record type (default 2)")
	fmt.Fprintln(stdout, "  -concurrency int       Number of concurrent workers (default 10)")
	fmt.Fprintln(stdout, "  -rate-limit int        Requests per second, 0 means unlimited (default 0)")
	fmt.Fprintln(stdout, "  -resolver string       Custom resolver IP:PORT")
